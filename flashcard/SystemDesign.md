...
Question 1) Zookeeper & Schema Registry?
Answer: Zookeeper: Manage cluster, store metadata.
...
Question 2) Why is replication across multiple Kafka Servers needed in production environment?
Answer: To compensate for the failure of any one.

Question 3) White-Box Testing?
Answer: Involves testing the internal structure, code and logic of a software application.
Usage: It is used to ensure that code is working correctly and efficiently, focusing on code coverage and path analysis.
Advantage:
a) Can identify complex logic errors, security vulnerabilities, and code optimization opportunities.
b) Can provide a higher level of coverage and uncover hidden defects.
Disadvantages:
a) Testers needs programming knowledge.

Question 4) Which options will increase the throughput on a Kafka producer?
Answer:
a) Increase batch size.
b) Setting a compression type as snoppy.
c) Increase linger-ms.

Question 5) Where to fire an event? Microservice architecture?
Answer: In Application Service. Domain Layer should not know about how to fire the event.

Question 6) When to choose synchronous vs asynchronous communication?
Answer:
Synchronous:
...
Question 7) What is a Microservice?
...
Question 8) What is a measure for performance?
Answer:
a) Latency.
b) Graceful degradation.
c) Throughput.
d) Response Time.
e) Short tail latency (narrow latency distribution).
f) Instructions per cycle.

Question 9) What is a maximum number of consumers that can work on Kafka topic?
Answer: Maximum number of consumers that can work on Kafka is equal to a partition number.
The additional consumer will be idle won't do any job.

Question 10) What does Thrashing mean in the context of computer science?
...
Question 11) What are the factors that influence the technical Micro Service?
Answer: Communication, Resilience, New Microservices.
Operations:
a) Logs. 
b) Metrics.
c) Configuration.
...
Question 12) What are some roles of OS?
Answer:
a) Memory management.
...
Question 13) What are arguments for a higher number of partitions?
Answer: More opportunity for concurrency.

Question 14) Web Service W3C Definition?
Answer: Software system designed to support interoperable machine-to-machine interaction over network.
3 Keys:
...
Question 15) We have a computer with a single core?
Answer: We can achieve concurrency using multiple threads.
We can achieve concurrency using multiple processes.
We can only achieve parallelism with multiple cores, but by multitasking, the OS can achieve concurrency
using just one core by the means of multithreading.

Question 16) Virtual Machines?
Answer: Virtual Machines are simulated computers that run on the same physical hardware.
For the OS and app, virtual machines look exactly like a physical server.

Question 17) Test case? 
Answer: A test case is a detailed and documented set of instructions or steps that a tester follows to execute a 
specific test scenario and evaluate whether a software app behaves as expected.
a) Test Case ID.
b) Test Case Title / Description.
c) Test Objective.
d) Preconditions.
e) Test steps.
f) Expected Results.
...
Question 19) System Design step-by-step process?
Answer: 
First: Ask Questions to capture:
a) Functional Requirements.
b) Non-functional Requirements. 
c) System Constraints.
Second: Define the System APIs.
Third: Create a Software Architecture Diagram to fulfill the Functional Requirements.
Fourth: Refine the Software Architecture Diagram to address the Non-Functional Requirements.

Question 20) System Design Software Extensibility Architecture Pattern?
Answer:
a) Sidecar & Ambassador.
b) Anti-corruption Adapter pattern.
c) Backends for Frontends pattern.

Question 21) System Design Latency?
...
Question 22) System Design Cloud Scalability Pattern?
Answer:
a) Load Balancing.
b) Pipes and Filters.
c) Scatter Gather.
...
Question 23) System Design Cloud Performance Pattern for Data Intensive Systems?
Answer:
a) Map Reduce.
b) The Saga Pattern.
...
Question 24) What is a symmetric encryption?
Answer: Single key for encryption and decryption.

Question 25) Process data structures?
Answer: Process Table, Thread Table, File Table, IPC (Inter Process Communication), I/O (Input/Output).

Question 26) What are difference between Strategic DDD and Tactical DDD?
Answer: Strategic DDD: Introduces boundaries from domain model. Single bounded context per each domain.
What is a Domain? Operational area of your application. e.g. online food ordering.
Unbounded Context: Central Pattern in DDD. Boundary within a Domain.
...
Question 27) Difference between SQL and NoSQL?
Answer:
SQL table-based relational DB.
a) Data is stored in precisely defined structures and the relationships between them are represented using foreign keys.
b) Good at querying multiple tables and large data sets, as well as maintaining transaction consistency and security.
NoSQL allows you to store data in various formats, such as documents, graphs, key-value or column.
a) More flexible and scalable than SQL.
b) Good for projects with a lot of load data.
c) Error-resistant because the data is replicated.

Question 28) What are different Specialized Storage Paradigm?
Answer:
a) Blob Storage: GCS or S3.
b) Time Series Database: Special kind of database optimized for storing and analyzing time-indexed data. 
Data points that specifically occur at a given moment in time. InfluxDB, Prometheus and Graphana.
...
e) Spatial Database: Storing spacial data like locations on a map. Spatial indexes like quadtrees.

Question 29) What is SOAP?
Answer:
Data Format:
a) SOAP XML Request.
b) SOAP XML Response.
...
a) SOAP over MQ.
b) SOAP over HTTP
Service Definition: WSDL defines:
a) Endpoint.
b) All Operations.
c) Request Structure. 
d) Response Structure.

Question 30) What is Smoke Testing?
Answer: Smoke Testing also known as build verification testing, involves running a subset of test cases to verify 
that core functionalities of a software applications are operational after a new build or release.
Quick feedback on whether build is stable enough for more extensive testing.
Identifies major issues. Prevents wasting time on comprehensive testing.

Question 31) Set Based Consistency CQRS?
Answer: How to check if record already exists in a database table?
How to check if User already exists? Lookup database table.
...
Question 32) Service Discovery and LB in Microservices?
...
c) Load Balancers: Google Cloud LB

Question 33) SOAP Service Definition?
Answer:
...
Question 34) What are Self-Contained Systems (SCS)?
Answer: SCS is a type of microservice that specifies elements of macro architecture.

Question 35) What is Security Testing?
Answer: Security testing encompasses a range of testing techniques designed to assess the confidentiality, integrity, 
authentication, authorization and availability of software applications.
a) Risk Mitigation.
b) Compliance: PCI DSS, GDPR.
c) Customer Trust.

Question 36) What is a role of Schema Registry in Kafka?
Answer: Forces producer and consumer to use a valid schema.

Question 37) Scatter Gather Pattern Use Cases?
Answer: Scalability pattern. Use Cases:
a) Search Services.
b) Hospitality Services.

Question 38) Example of Scalable Unstructured Data Storage Cloud and Open Source Solutions?
Answer:
a) Amazon S3.
b) Google Cloud Storage.
c) Azure Blob Storage.

Question 39) Use cases for SAGA Pattern?
Answer: Distributed long-running transactions across services. Used for Long Lived Transactions (LLT).
Chain of local ACID transactions to finalise a long-running transaction across service.
Different kinds:
a) Choreography-Based.
b) Orchestration-Based.
...
Question 40) Robustness ways to ensure?
Answer:
a) Error Handling.
...
Question 41) Robust system, what does it means?
Answer: A robust system is able to operate efficiently, even in the presence of errors or other problems.
Analogy to a 2-engine airplane.
Key Features: Fault Tolerance, High Availability, Scalability, Resilience, Security, Efficiency, Managability,
Flexibility.

Question 42) REST key abstraction?
Answer: A resource has an URI: /user/panda/todos/1.
A resource can have different representations: XML, JSON, HTML.

Question 43) What are Rest API Resources - Best practices?
Answer:
...
Question 44) What are Rest API Important Concepts?
Answer: 
a) HATEOAS
b) Statelessness: Scalability and High Availability
c) Cacheability: Performance
d) Named Resource: Simple Resource, Collection Resource

Question 45) What are Replication and Sharding?
...
Copy in multiple node, each one are identical, BANKS, HOTELS, TELECOMMUNICATION.
Sharding split big DB to smaller (shards) stored in different part of network.
Each node handles only subset of data. This allows to parallel data processing and faster reclaim data from big databases.
Big performance and scalability, merchant systems, analysis of data.

Question 46) Ways to replaying events in Kafka?
Answer: Create a new consumer group. It will read the events from start by default.

Question 47) Remote Procedure Call Technologies?
Answer:
a) gRPC.
b) Apache Thrift.
c) Java Remote Method Invocation (RMI).

Question 48) What is Redis?
Answer: An in-memory key-value store. Does offer some persistent storage options but is typically used as a really 
fast the best effort caching solutions. Redis is also used to implement rate limiting.

Question 49) What is Reactive Programming?
Answer:
...
Question 50) What is a Quorum in Kafka?
Answer: Set the minimum number of brokers to create a network, prevent split brain.

Question 51) Describe Quality attribute Response Time?
Answer: Time between a client sending a request and receiving a response.
ResponseTime = Processing Time + Waiting Time.

Question 52) Describe Push model in Postgres?
Answer: Push model where Postgres notifies the other apps for each change.

Question 53)  Difference between Processes and Threads?
Answer: Process independent execution unit, contains own address space, file descriptor, memory space, virtual devices.
Threads use system resources to which belongs. In process multiple threads, thread can have one process.
...
Question 54) Describe Processes in Operating System?
...
Question 55) Principle of service discovery in microservice architecture. Example of tools?
Answer: Enables services to find and communicate with each other.
...
c) Dynamic Updates.
d) Resilience.
Tools: consul, etcd, zookeeper, kubernetes, netflix eureka, dns-based (netflix ribbon)

Question 56) What is POSIX (Posix)?
...
Question 57) Popular Service Discovery Tools and Techniques?
Answer:
...
Question 58) Describe Pipes and Filters pattern?
Answer: Scalability pattern.
Problem Solved: Tight Coupling, Hardware Restrictions, Low Scalability.
Benefits: Use different programming languages, High Scalability, High Throughput.
Use Cases: Digital adv business, IoT data processing, Ingest / process Video and Audio.

Question 59) Describe Peer-to-Peer Network?
...
Question 60) What is Page Cache?
Answer: Consist of physical pages in RAM, corresponds to physical blocks on DISC (Disc).

Question 61) Describe Outbox Pattern?
...
b) Change Data Capture: Listen transaction logs.
Keep track of SAGA and order status in Outbox Table. Ensure idempotency: Do not consume same data. Optimistic locks and 
BS Constraints: Prevent Data Corruption.

Question 62) Describe OS Threads?
...
Question 63) Describe OS livelock?
...
Question 64) Describe OS Kernel?
...
key press, end of execution of app. Energy management - hibernation.

Question 65) Describe Operating Systems Virtualization?
...
Question 66) What is OpenID Connect?
Answer: For authentication of user by using an ID token.

Question 67) Describe OAuth 2?
Answer: For delegated authorization by using an access token.
Terminology:
a) Resource Owner.
b) Client.
...
Question 68) What are NoSQL use cases?
Answer: Superior when it comes to query speed. Perfect choice for caching. Handling real-time big data. 
Data is not structured.

Question 69) NoSQL Solutions?
Answer:
a) Key / Value Store: Redis, Aerospike, Amazon DynamoDB.
b) Document Store: Cassandra, MongoDB.
c) Graph Database: Amazon Neptune, Neo4j

Question 70) Non-Functional Requirements tools?
Answer:
...
Question 71) Non-functional requirements example?
Answer:
...
Question 72) Microservices, strategies and patterns to ensure high availability and fault tolerance?
Answer: Strategies and Patterns:
a) Redundancy and Replication.
b) Circuit Breaker Pattern: Netflix Hystrix, resilience4j.
c) Retry Mechanism.
d) Health Checks and Monitoring: Prometheus, Graphana.
e) Stateless Services.
f) Distributed Tracing: Zipkin.
g) Graceful Degradation.
h) Chaos Engineering - Netflix Chaos Monkey.

Question 73) Microservices, micro and macro architecture decisions?
Answer: 
...
Question 74) What are Microservices trade-offs?
Answer: Robustness is the goal. Robustness does not matter. 

Question 75) What is Microservices scalability requirements?
Answer: Stateless.

Question 76) How to achieve Microservices Robustness?
...
Question 77) Describe Microservices?
...
Question 78) Microservice stronger isolation?
...
Question 79) Message Brokers Solutions & Cloud Technologies?
...
Question 80) What is Megabyte (Mb)?
Answer: 10 to the power of 6 or 1000000 bytes.

Question 81) Mebibyte (Mi)?
Answer: 2 to the power of 20 or 1048676 bytes.

Question 82) Describe Map Reduce Pattern?
Answer: Performance Pattern. Dominant Factors: Parallelism, Distribution of data, Scheduling execution,
Result Aggregation, Handling & recovering from errors.
A popular framework for processing very large datasets in distributed setting efficiently, quickly, and in fault-tolerant manner.
Contains of 3 main steps:
...
Question 83) Managing API Documentation and Versioning Challenges?
Answer:
a) Documentation Tools: Swagger, OpenAPI. 
b) Clear Guidelines.
c) Using Versioning Strategies: URI: /v1/resource, header: Accept: application/vnd.myapi.v1+json 
d) Documentation as Code.
e) Testing and Validation. 
f) Deprecation Policies.
g) Feedback Loop. 
h) Collaboration.
...
Question 85) Load Balancing Solutions & Cloud Technologies?
Answer: HAProxy, NGINX
Cloud Based LB Solutions:AWS Elastic LB (ELB), GCP Cloud LB, Microsoft Azure LB.
GSLB Solutions: Amazon Route 53, Google Cloud Platform LB & Cloud DNS, Azure Traffic Manager.

Question 86) Load Balancing Implementation Considerations?
Answer: Round Robin: For stateless applications. Sticky Session / Session Affinity. 
Least Connection: Task associated with long term connections (SQL, LDAP).

Question 87) Leader Algorithm?
...
Question 88) Latency and Throughput?
Answer: Latency and throughput are the two most important measures of the performance of a system.
Latency: Time required to send packet from one computer to other in computer network. Measured in ms. 
Check inter-node propagation time.
Throughput: Quantity of data, which can be send by network in a time. Measured in bps, kbps. 
Connection throughput limitation, use of resource be app or quality of connection.

Question 89) Key-Value Store?
...
Question 90) Key differences between microservice and monolithic architecture?
...
Challenges: Complexity, Communication, Communication overhead, Data management, Testing, Operational Overhead, 
Security, Service Discovery.

Question 91) Kafka what are arguments for a lower number of partitions?
Answer: A Lesser memory consumed on the client. It takes less time to find a leader for each partition if the node fails.

Question 92) What are Kafka Topics?
...
Question 95) Kafka snoppy compression?
Answer: Arranges compression ratio for higher compress/decompress speed.

Question 96) Kafka scalability ordering?
...
Question 102) Kafka Persistence?
Answer: Kafka is a micture of messaging system and data storage solution. By default retention for records is seven days.

Question 103) Kafka Partitions?
Answer: Topics are divided into partitions.
Partitions allow strong guarantees concerning the order of records, but also parallel processing.
For each partition, the order of the records is preserved. There is no guarantee of order across partitions.
Across partitions, processing can be parallel. Basically, a partition is just a file to which records are appended.

Question 104) Kafka partition key?
Answer: Set the target partition for a message.

Question 105) Kafka Log compaction?
Answer: All records with the same key are removed, except for the last once. 
Only very last update to the order remains available in Kafka.

Question 106) Kafka linger ms?
Answer: Add a delay in case of light load.

Question 107) Kafka Leader and Follower?
...
Question 108) Kafka High Availability?
Answer: Eliminates single point of failure.

Question 109) Kafka consumer groups?
...
Question 110) Kafka Consumer basics?
...
Kafka distributes partitions to consumers based on the consumer group identifier.
Each partition is assigned to single consumer in a consumer group.
Different consumer groups can read the data from the same partition?
Set partition number equal to consumer number.
Set poll timeout to a value not too big to prevent blocking indefinitely, not too small to prevent CPU stale.
Delivery semantics: at least once, at most once, exactly once.
A partition can have only one consumer.
A consumer can read multiple-partitions.
Kafka Consumer Concurrency Level: Threads -> Processes -> Machines

Question 111) Kafka Commit?
...
Can guarantee the delivery of messages. The Consumer commits the records it has successfully processed. 
If an error occur, it does not commit the record and tries to process it again (retry).

Question 114) Kafka batch size?
Answer: Tune for higher throughput.

Question 115) Kafka basics?
...
Question 116) Kafka Avro as data format?
Answer: Avro is a binary protocol, but also offers a JSON-based representation.
Avro support schema, meaning that each dataset is saved or sent together with its schema.
Schema evolution, new fields can be added when default values are defined so that the old data can be converted into 
the new schema by using the default value.

Question 117) Kafka Architecture?
...
Question 118) Kafka APIs?
Answer: The producer API Servers to send data. The consumer API servers to receive data. 
Streams API servers to transform the data.

Question 119) Kafka and Programming on Kafka?
...
To create a Kafka Consumer with Spring, using a KafkaListener on a simple method is enough.
If my Kafka topics has n partition, I cannot use more than n threads to consume the data on topics.
Kafka Consumer has a max poll records property that limits the number of records returned for a single poll.

Question 120) Kafka advantages?
Answer: High throughput. Low latency. Replication and can scale. Stores message history 
(approaches like event sourcing are possible).

Question 121) Kafka AdminClient?
Answer: Manage and inspect brokers, topics and configurations.

Question 122) Kafka Acknowledgements acks?
Answer: all: wait ack from all replicas. 1: wait only current brokers ack. 0: no ack.

Question 123) How to ensure high performance in distributed systems?
...
Question 124) How to simplify distributed systems?
...
Question 125) How is context switch invoked by OS and hardware?
...
Question 126) What is IP protocol?
...
Question 128) Data Integrity - methods of ensuring?
Answer: Data Validation. Data Replication. Backups. Data encryption.
Data consistency: Ensuring that data is the same between servers, using techniques such as two-phased-commit 
or a distributed consensus algorithm. Data Audit. Error detection and correction, checksums or correction codes.
Access control.

Question 129) Independent System Architecture Principles?
Answer:
...
Question 130) In Kafka-based system, N consumers exists and M partitions exists. What is possible?
Answer: N (consumers) greater than M (partitions) not possible
Multiple consumers cannot read from the same partition.

Question 131) Importance of test-driven-development TDD?
Answer Importance:Quality Assurance, Specification, Rapid Feedback, Refactoring Safety.
TDD workflow: First Step Write a Test. Second Step Write Code. Third Step Run Test. Fourth Step Refactor Code. 
Fifth Step repeat.
...
Question 132) Importance of API Gateway in microservices architecture?
Answer: Centralized Entry Point. Routing. Load Balancing. Authentication and Authorization. Rate Limiting.
Example: Amazon API Gateway.

Question 133) Idempotent Operations?
Answer: An operation doesn't have any additional effect on the result if it is performed more than once.

Question 134) How Service Discovery Facilitates Communication?
Answer: Dynamic Addressing. Load Balancing. Health Checking. Failure Recovery.

Question 135) Hexagonal Architecture, Primary Adapters, Secondary Adapters?
...
Question 137) Functional vs. Regression testing?
...
Question 138) Exponential Back Off Policy?
Answer: Increase wait time for each retry attempt.

Question 139) Exploratory Testing?
Answer: Exploratory testing is an unscripted testing approach where testers simultaneously design, execute 
and adapt tests based on their understanding of the software. Encourage critical thinking.

Question 140) Explain various methods or protocols that can be used for inter-service communication?
Answer: 
One, HTTP/Restful API: Simple, easy to implement, compatibility are required.
Two, gRCP: Micro in different programing languages. Internal communication within private network.
Three, Message Queues: RabbitMQ, Apache Kafka. Asynchronous communication.
Four, Graph QL: Query language for APIs that enables clients to request only the data they need.
Five, WebSocket: Full-duplex, bi-directional communication channel over a single, long-lived connection. Chat, gaming.
Six, Service Mesh (Istio, Linkerd): Dedicated infrastructure layer for handling service-to-service communication. 
Manage and secure communication in complex micro-services ecosystem.

Question 141) Explain the concept of distributed transactions and the two-phase commit (2PC) protocol?
Answer:
Distributed Transactions: Multiple operations across distributed systems or microservices, where each operation 
...
One, Prepare Phase: Coordinator asks all participants if they are ready to commit. 
When agrees, they respond with "prepared" message.
Two, Commit Phase: If all participants respond positively in the prepare phase, coordinator sends a "commit" message 
to all participants. If any participant response negatively or not at all, coordinator sends rollback message to undo transactions.
...
Question 142) Explain the concept of circuit breakers in microservices, and how do they contribute to system resilience?
Answer:
Enhance fault tolerance and resilience. Can help prevent cascading failures and improve system stability.
One, Closed State. Two, Open State. Three, Half-Open State.
Improve fault tolerance and prevent cascading failures when interacting with remote services. 
They provide a mechanism to handle service degradation gracefully and protect the overall system's reliability.

Question 143) Execution Orchestrator Pattern?
...
Question 144) Eventual Consistency in Distributed Systems?
Answer: Allows for temporary inconsistencies between copies of data, but guarantees that those will be resolved. 
Availability and Partition Tolerance are crucial.
Ensuring Data Consistency:
One, Use Idempotent Operations. Two, Implement Data Versioning. Three, Conflict resolution strategies "least write wins", "merge".
Four, Asynchronous Communication. Five, CQRS Pattern. Six, Event Sourcing. Seven, Monitoring and Alerting.
Eight, Testing. Nine, Documentation and tracing.

Question 145) Eventual consistency?
...
Question 146) Event Store?
Answer: Store data as immutable events.

Question 147) Event Driven Architecture?
...
Question 148) ETL Extract, Transform, Load?
...
Extract: extracting data from source, Apache NiFi, Kafka Connect. Transform: processing according to Apache Spark, 
Apache Beam rules. Load: transferring data to the target system. Apache Kafka, Amazon S3, Elastic-Search. 

Question 149) Etcd Database?
Answer: Etcd is a strongly consistent and highly available key-value store that often used to implement 
leader election in system.

Question 150) Domain Driven Design DDD?
...
Question 151) Domain Driven Design?
...
Question 152) DNS Queries?
Answer: Human Readable -> IP address.
Command One, nslookup www.google.com
Command Two, dig www.google.com
Process: One, Query.Two, DNS check cache. Three, Searching: a) Send to root DNS for domain. 
b) Send to one of services responsible for domain. c) Receive IP address from one of servers.
Four, DNS save in cache. Five, Return address to client.

Question 153) Distributed tracing in microservices architecture and why it's valuable for monitoring and troubleshooting?
...
of request paths and the identification of performance bottlenecks.
Example, Jageor.

Question 154) Distributed File System?
...
Question 155) Differences between synchronous and asynchronous communication in microservices?
Answer:
Synchronous: Client sends a request and waits for a response before continuing communication.
Suitability, Client requires immediate response, and can tolerate latency. Querying DB. 
When strong consistency is essential.
Asynchronous: Client sends a request and continues its operation without waiting for an immediate response.
Suitability: Low-latency is critical. Long-running tasks, background processing, event-driven and decoupling services.
Scalability, responsiveness and fault tolerance are priorities.
...
Question 156) Difference between smoke testing and sanity testing?
...
Question 157) Difference between functional and non-functional testing?
Answer:
Functional: Verifies that the software apps functions and features work accordingly to the specified requirements and specifications.
Consists of, Unit Testing, Integration, System, User Acceptance Testing (UAT) "what".
...
Question 158) Describe the typical phases of software testing lifecycle STLC?
Answer:
One, Requirements Analysis.Two, Test Planning. Three, Test Design. Four, Test Environment Setup.
Five, Test Execution. Six, Defect Reporting and Tracking. Seven, Defect Resolution and Retesting.
Eight, Test Closure.

Question 159) What is a Dependency Inversion?
Answer: High level modules should not depend on low level modules. Both should depend on abstraction.

Question 160) What is Debezium?
...
Question 161) DDD Strategic Design & Common Patterns?
Answer: 
Customer / Supplier Pattern: Supplier is upstream and consumer is downstream. For example uses the model of the order process. 
However, payment defines requirements for the order Process.
Conformist Pattern: Bounded context simply uses a domain model for another bounded context.
The anti-corruption layer: Bounded context does not directly use the domain model of the other bounded context.
The Separate Way Pattern: Bounded Contexts are not related at the software level although a relation would be conceivable.
The Shared Kernel Pattern: Describes a common core that is shared by multiple bounded contexts. Microservices anti-pattern.
The Open Host Service Pattern: Bounded context offers a generic interface with several services. 
Frequently found at public APIs on the internet.
The publish language model: Is a domain model accessible by all bounded contexts. For example standard format like 
EDIFAC for transaction between companies.

Question 162) Detailed testing plan?
...
Question 163) Data Durability?
Answer: Data replication - Data Replication. Backups - Data Backups. RAID Storage - RAID Storage: Data stored on several 
disks and can survive disk failure. Data Durability Guarantees: Durability guarantees such as write-ahead logging or snapshotting.
Error detection and correction: checksums or error-correction codes. Data composition: two-phase-commit or distributed 
consensus algorithm. Durable message queues. Cloud solutions.

Question 164) Differences between CRUD and CQRS?
Answer:
One, Responsibility: 
CRUD: read + write together (Data Access Layer). CQRS: segregated responsibility Commands Command layer, Queries Query Layer
Two, Operations:
CRUD: 4 create, read, update, delete. CQRS: commands change state, queries read.
Three, Data Model:
CRUD: one model. CQRS: split model, e.g. DB optimized for writing or reading.
Four, Scalability: 
CRUD: all operations scale together. CQRS: independent scaling.
Five, Events:
CRUD: no natural mechanism for tracking changes. CQRS: Event Store.

Question 165) CQRS Type of Messages?
...
Query: Express the desire for information, for example FindProductQuery, GetUserQuery.
Event: Represent a notification that something relevant has happened, for example ProductCreatedEvent, ProductUpdatedEvent.

Question 166) Describe CQRS?
...
Question 167) Context switch system calls?
Answer:
shed.h: Header file containing definitions of structures and functions related to process and thread scheduling.
clone(): A system call that creates a new thread or process.
pthread_create(): C function, creates a new thread.
pthread_yield(): C function, allows a thread to voluntarily donate CPU time to another thread waiting for execution.
sem_wait() and sem_post(): C function, synchronization by using semaphores.
mutex_lock() and mutex_unlock(): C function, synchronization by using mutexes.

Question 168) Context Switch Algorithms?
Answer:
One, Round-Robin: Divides CPU time to each task in order with a fixed time cycle.
Two, Priority-Based: Processor time is allocated based on priorities.
Three, Shortest Job First: Allocates CPU to jobs based on their length.
Four, Guaranteed Scheduling: Ensures that each task receives a specific amount of CPU time.

Question 169) Containerization and its significance in microservices deployments?
Answer: Containerization is the process of packaging an application and its dependencies, including libraries 
and config files, into a single, lightweight unit called container.
Significance in Microservices: Consistency, Isolation, Portability, Scalability.

Question 170) What is a Container Orchestration?
Answer: Automated management of containerized apps, including tasks like deployment, scaling, LB and service discovery.
...
Question 171) What is a Consistent Hashing?
...
Question 172) Concept of service mesh in microservice architecture?
...
Strengthens security with features like authentication, authorization and encryption.
Benefits of service mesh: Traffic management: LB, circuit braking, routing. Observability. Security. 
Resilience: Circuit Braking, Retries. Policy Enforcement
Tools Example: Istio.

Question 173) Concept of event-driven architecture in the context of microservices?
...
Question 174) Concept of distributed database?
...
access and redundancy. Enable to access data while maintaining CAP.
Supports horizontal scalability. Enforce fault tolerance. 
Benefits of Distributed Databases: Scalability. High Availability: Data replication. Reduced Latency.
Data Consistency: Thorough quorum-based systems or eventual consistency model.
Challenges: Complexity. Latency.
Consistency Trade-offs: Strong consistency across all nodes at cost of increased latency or decreased availability.

Question 175) Concept of containerization in the context of microservices architecture using Kubernetes?
...
Ensures consistency between dev, testing and prod envs. Isolates apps, preventing conflicts between dependencies.
Benefits: Portability. Resource Efficiency. Consistency. Isolation.
Kubernetes is an open-source container orchestration platform that automates the deployment, scaling and management 
of containerized apps. It provides features like load balancing, scaling, self-healing, and rolling updates.
Containerization. Kubernetes Deployment: Management desired state of an app, num of replicas, resource allocation, scaling policies.
Scaling: Based on resource utilization. Load Balancing. Rolling Updates.

Question 176) Common Service Discovery Methods and Tools?
Answer: 
DNS-Based Service Discovery: Netflix Eureka, Spring Cloud Netflix.
Key-Value Stores: consul, etcd, zookeeper.
API Gateway or Reverse Proxy: Nginx, HAProxy, Cloud based API gateway.
Sidecar Proxies: Envoy, Linkrd, Istio.
Cloud-Based Service Discovery: AWS ELB, Google Cloud Load Balancing.

Question 177) Command Query Responsibility Segregation (CQRS)?
Answer: Separate read and write operations. Better performance on read part using right technology for reading, 
and preventing conflicts with update commands. Scale each part separately. Leads to eventual consistency.
Eventual Consistency: The data will reach to Read Store with a small delay.

Question 178) Clean Architecture?
...
Question 179) Circuit Breakers in context of microservice architecture, how contribute to the resilience?
...
Question 180) Choreography-Based Saga compensation flow?
Answer:
Initial Flow: Create Order operation. Reserve Product operation.
Compensating Transaction: Cancel product reservation. Reject order.
Initial Flow: 1,2,3,4,5,...,n. Compensating Transactions: n,...,5,4,3,2,1.

Question 181) Choreography Pattern?
Answer: Scalability. Helps scale a complex flow of business operations. Event Driven Architecture using choreography.
Communication is asynchronous.
Example: (Saga) SAGA.

Question 182) What is a Change Data Capture?
...
Log is called as Write Ahead Log (WAL). Provides near real-time data with very low overhead by listening 
the Transaction Logs and sending to target source (Kafka) immediately.

Question 183) Challenges and solutions related to maintaining data consistency?
Answer:
Challenges: Data Ownership. Distributed Transactions. Data Synchronization. Data Duplication.
...
Question 184) CDN Solutions & Cloud Technologies?
Answer: Cloudflare. Fastly. Akamai. Amazon Cloud Front. Google Cloud Platform CDN. Microsoft Azure Content Delivery Network.

Question 185) CDN Publishing Strategy?
...
Question 186) Can you explain challenges associated with data management in microservices architecture, strategy + pattern?
Answer:
Challenges: One, Data Consistency. Two, Data Ownership. Three, Data Integrity. Four, Data Access Pattern. Five, Data Volume.
Strategies: Database per service. Shared Database. Event Sourcing. 
CQRS: Separate the command (write) and query (read) responsibilities into different components. 
Command side updates data, and query side retrieves data.
...
Question 187) What is Caching?
Answer: Hardware or software that stores data. They are used to store responses to network requests, 
but also the results of long computations. Data may be out of date (permanently), e.g. The DB has been updated but 
...
Also called PoP (Point of Presence).
Example, Cloud Flare.

Question 188) Business Logic in Clean Architecture?
...
Question 189) What is a Bridge network?
Answer: Creates a network to connect with hostnames.

Question 190) Bounded Event DDD?
Answer: Domain events: For the communication between bounded context.
Represents something that happened in the domain.
For example shopping cart. This event is triggered by bounded context order process,
and it is received by the bounded contexts shipping and payment to initiate shipping and invoicing of the order.

Question 191) Boundary Testing and its importance in software testing?
...
Accuracy. Security. Quality.

Question 193) Black-Box vs White-Box testing, when use?
...
Question 194) Black-Box Testing Example?
...
Question 195) Black-Box Testing?
...
Question 196) Describe a Bankers Algorithm?
Answer: Algorithm used in OS to avoid deadlocks in a multi-processors systems.
Assumptions:Process has to declare max number of resources which will need.
All resources assigned to process at the beginning, but can be released and assigned to other processes.
Maintains actual lists of available and used resources.
If process want to acquire a resource, algorithm checks whether allocation won't lead to deadlock.
Simulates assign resources and checks.

Question 197) What is Avro?
Answer: Compact, fast, binary data format that relies on schemas.

Question 198) Availability how is it obtained?
...
Question 199) Automated Testing in software development and its advantages?
Answer: Advantages: Efficiency. Consistency. Regression Testing. Coverage. Continuous Integration.

Question 200) Asynchronous Microservice?
...
Question 201) Describe Distributed architecture?
Answer: Many devices and machines connected via a network.
Multiple devices work together to perform a single task or set of tasks, sharing resources and information among themselves.
Client-server architecture. Peer-to-Peer Architecture. Microservice architecture. Cloud-based architecture.
Distributed architecture allows for better scalability, reliability, availability and fault tolerance, 
the load distributed among devices and resources can be shared.
Easier to manage and update. Challenges like network latency, network errors, data consistency, security.

Question 202) API versioning in microservices architecture?
...
Allows clients to choose the API Versions.
API versioning strategies: URI Versioning. Header Versioning. Query Params Versioning. Media Type Versioning.

Question 203) API Gateways (Definition + Challenges)?
...
Question 204) API Gateway Solutions & Cloud Technologies?
Answer: Netflix Zuul. Amazon API Gateway. Google Cloud Platform API Gateway. Microsoft Azure API Management.

Question 205) API Definition Example?
Answer: REST API over HTTP: One, Identifying Entities. Two, Mapping Entities to URIs. 
Three, Defining Resources Representation. Four, Assigning HTTP Methods to operations on resources.

Question 206) Apache Kafka Performance?
Answer: Partitioning: Data separated into multiple partitions.
Replication: Replicating partitions across multiple nodes, improving system reliability and availability.
Scalability: Scalable vertically and horizontally. You can increase the number of nodes in the cluster to increase 
...
Question 207) Apache Kafka data formats?
...
Question 208) Apache Kafka commit offset?
...
It ensures that each consumer reads the message only once.
Thanks to this, when a new consumer appears, Kafka knows what message has already been processed.
Automatic and manual committing of offsets.

Question 209) Apache Avro?
...
Schemas are independent of the programming language.
Application: distributed systems, data warehouses, stream processing systems, Big Data.
Apache Hadoop, Apache Kafka, Apache Spark. Advantages: Compact data format. Independence from programming language.
Availability of many tools: rich library in various programming languages.
In short, Avro is a system that allows you to move data between different programming languages and platforms, 
which is especially useful in distributed systems and Big Data.

Question 210) What is AES?
Answer: Standard for Advanced Encryption Standard.
AES is a widely used encryption standard that has three symmetric-key algorithms (AES-128, AES-192, and AES-256).
Off note  AES considered to be the "gold standard" in encryption and is even used by the U.S. National Security Agency 
to encrypt top secret information.

Question 211) Advantages and challenges of using microservices architecture in the development of distributed systems??
Answer:
Advantages of microservices: One, Scalability. Two, Modularity. Three, Flexibility. Four, Rapid Development.
Five, Fault isolation
Challenges:
One, Complexity. Two, Inter-Service communication. Three, Data Consistency. Four, Deployment and Monitoring.
Five, Testing.

Question 212) What are some of the purposes of a message broker?
Answer: 
Temporarily/permanently store messages between a producer and a consumer
Decouple the producers from the consumers.
Provides buffering capabilities to absorb sudden traffic spikes.

Question 213) What is the primary purpose of a load balancer in a system?
Answer: Distribute traffic among a group of servers running the same service.

Question 214) What are some potential drawbacks of DNS-based load balancing?
Answer:
It exposes server IP addresses to clients.
It does not actively monitor the health of servers.

Question 215) What is the key difference between hardware load balancers and software load balancers?
Answer: Software load balancers run on general-purpose computers, while hardware load balancers are dedicated devices.

Question 216) What is the main benefit of Global Server Load Balancers (GSLB) regarding system performance?
Answer: They provide intelligent routing based on various criteria, including geographical location and server 
load in a particular region.

Question 217) What is the role of a Global Server Load Balancer (GSLB) in disaster recovery situations?
Answer: It routes users to different locations for higher availability.

Question 218) What is the most correct statement regarding an API Gateway?
Answer: An API Gateway routes user requests to different microservices based on the request's API.

Question 219) Please select the features that an API Gateway typically provides.
Answer: Rate Limiting. Protocol Translation. Monitoring. Authorization.

Question 220) What are the benefits of making an application stateless?
Answer:
...
Question 221) What is the main purpose of a CDN?
Answer: To speed up the delivery of static content to end-users

Question 222) How does the CDN solve the latency problem when the user is far from our system's data center?
...
Question 223) How does a CDN improve the availability of our system?
...
Question 224) Our security experts were notified that our system is under a DDoS (Distributed Denial of Service) Attack.
Which of the below options will be an effective measure to reduce our vulnerability to it?
Answer: 
a) Incorporate a CDN.
b) Add rate-limiting to the API Gateway.
Incorporating a CDN will help distribute the attack across a very large number of edge servers outside of our organization.
Additionally, adding rate-limiting will limit the number of traffic our services get at a given time period, 
eliminating the possibility of one of our services getting overloaded, which may lead to a cascade of failures.

Question 225) What is a database transaction?
Answer: A set of operations that, for an external observer, appears as a single operation that either commits or aborts.

Question 226) What does Consistency in the context of an ACID transaction mean?
Answer: 
a) A transaction that was already committed will be seen by all future queries/transactions.
b) A transaction doesn't violate any constraints we set on our data.

Question 227) Durability?
...
Question 228) statements regarding SQL and NoSQL databases?
Answer: 
a) NoSQL databases typically use the same data structures as most programming languages, while SQL databases do not.
b) SQL databases are optimized for efficient storage. NoSQL databases are optimized for efficient queries.
c) NoSQL databases are easier to scale horizontally than SQL.

Question 229) Which database operations are typically more limited in NoSQL databases than in SQL databases?
Answer: 
a) ACID Transactions.
b) Join operations.

Question 230) Example of use cases for a graph database?
...
Question 231) What tradeoff do we make by introducing indexing to a database?
Answer: We get faster read operations at the expense of additional space and slower insert operations.

Question 232) What are the benefits of database replication?
Answer: 
a) Prevention of a Single Point of Failure.
b) Higher Availability.
c) Potential higher query throughput.

Question 233) What NOT a step in capturing functional requirements?
Answer: Deciding which programming language or technology is best for this situation.

Question 234) An API is a contract between..?
Answer: The engineers who implement the system and the clients that use the system

Question 235) Examples of idempotent operations?
Answer: 
...
Question 236) Why are idempotent operations preferable over non-idempotent operations?
Answer: Idempotent operations allow safe retries.
In a distributed system with unreliable components communicating with each other over an unreliable network, 
idempotent operations allow us to safely retry them without worrying about adverse effects.

Question 237) Examples of scenarios where we need to use API Pagination?
Answer:
...
Question 238) What is the correct statement when comparing between REST and RPC?
Answer: RPC looks and feels more like a local method invocation than a REST API call
A Remote Procedure Call uses Location Transparency and abstracts away all the complexities of the 
marshaling/unmarshaling of data and the management of the network communication between the client and the server.
Therefore, a typical RPC looks like a simple method call, no different than any other local method.

Question 239) Statements regarding RPC invocations when compared to local method invocations.
Answer: 
a) RPC calls are generally slower than local method invocations.
b) RPC calls are generally less reliable than local method invocations.
RPC invocations abstract away a network call to a remote service, requiring the data to be encoded in a binary format 
and waiting for the packages to travel through the network.
Therefore, RPC invocations are less reliable and slower than a local method call.

Question 240) Statement about REST API?
Answer: REST is a set of architectural constraints and best practices for defining APIs for the web

Question 241) One of the core principles of a RESTful API is statelessness.
What quality attribute does this principle provide to our system?
Answer: High Scalability.
When our application is stateless, we can run multiple instances of it, which allows us to scale our system horizontally.

Question 242) One of the core principles of a RESTful API is cacheability?
Answer: The server needs to implicitly or explicitly define a response as either cacheable or not cacheable by the client.
In a REST API, each response needs to be marked as cacheable or not cacheable so the client knows whether it is safe to cache that response.
When using HTTP to implement a REST API, any GET requests are cacheable by default.
Requests using the POST method are not cacheable by default but can be made cacheable.
DELETE and PUT methods are used for deleting and updating a resource, respectively, and are not cacheable.

Question 243) What does MTBF stand for?
Answer: Mean Time Between Failures.

Question 244) What does MTTR stand for?
Answer: Mean Time To Recovery.

Question 245) According to the definition of fault tolerance, what happens during a failure at a "fault-tolerant" system?
Answer: The system will continue to operate at the same or a reduced level of performance.

Question 246) What are the core principles of fault tolerance?
Answer: Failure Prevention. Failure Detection. Recovery from Failure.

Question 247) Which is an example of spatial redundancy in the context of failure prevention?
Answer: Running a service as a group of identical instances on different computers

Question 248) What is an example of a Time Redundancy in the context of fault tolerance?
Answer: Retrying a failed request multiple times.

Question 249) What is one drawback of synchronous communication between services?
Answer: It may result in high latency for long-running operations.

Question 250) What is one problem of synchronous communication solved by asynchronous, event-driven architecture?
Answer: Synchronous communication requires both the sender and receiver to maintain an active connection. 
And be available at the same time. Asynchronous, Event-Driven architecture doesn't.

Question 251) What is the difference between Multi-Tier Architecture and Multi-Layer Architecture?
Answer: Multi-Tier architecture involves the physical distribution of components across different runtime units running on different services.
In a Multi-Layer Architecture, the code is organized in separate logical units for better modularity but runs as a single runtime unit.

Question 252) Why is placing business logic in the Presentation Tier an Anti-Pattern?
...
Question 253) What is not a property of an event in an event-driven architecture?
Answer: Events map 1 to 1 to a row or record in a database
Events don't have to be mapped to records in a database.
...
Question 254) Describe the benefits of the CQRS Pattern in Microservices.
Answer:
a) In systems with complex business logic, where the operations to update data are significantly different from the 
operations to read data, CQRS allows for a cleaner separation of concerns by separating the Command part of the system 
and the Query part of our system into 2 separate services.
b) When the user request requires data from 2 different microservices to be joined, we can use the CQRS pattern to 
maintain the joined data in a read-only service.
c) Using CQRS, we can optimize our system for both read and write operations, which is otherwise very hard to achieve.
