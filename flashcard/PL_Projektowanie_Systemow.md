...
35) Zookeeper i Rejestr Schematów?
...
45) Jakie czynniki wpływają na techniczne mikroserwisy?
...
54) Wzorzec architektury rozszerzalności oprogramowania w projektowaniu systemu?
...
62) Jakie są różne specjalizowane paradygmaty przechowywania?
...
68) Czym są systemy samodzielne "S C S"?
...
78) Jakie są ważne koncepcje REST API?
a) "HateOas".
...
88) Opisz procesy w systemie operacyjnym?
...
105) Przykłady wymagań niefunkcjonalnych?
...
122) Opóźnienie i przepustowość?
...
co najmniej raz ("at least once"), maksymalnie raz ("at most once"), dokładnie raz ("exactly once"). Partycja może mieć tylko 
...
176) Wyjaśnij koncepcję wyłączników obwodowych "Circuit Breakers" w mikro-serwisach i jak przyczyniają się do odporności 
systemu? 
...
184) Projektowanie zorientowane na domenę "D D D"?
...
190) Różnica między testowaniem dymnym a testowaniem sanity?
...
196) Szczegółowy plan testowania?
...
201) Wywołania systemowe przełączania kontekstu?
...
207) Koncepcja architektury opartej na zdarzeniach (event driven) w kontekście mikro-serwisów?
...
211) Segregacja odpowiedzialności zapytań i poleceń "C Q R S"?
...
Prowadzi do ostatecznej spójności "Eventual Consistency". Ostateczna spójność: Dane dotrą do magazynu odczytu z 
niewielkim opóźnieniem.

212) Czysta architektura?
...
Zależności kodu źródłowego mogą wskazywać tylko do wewnątrz, w kierunku warstwy domeny. Niezależna od frameworku, 
...
Łatwiejsze testowanie logiki biznesowej poprzez tworzenie atrap zależności nawet przed podjęciem decyzji o szczegółach 
implementacji. Łatwe zastępowanie implementacji adaptera bez dotykania logiki biznesowej. Niezależny rozwój i wdrażanie 
różnych części.

213) Bezpieczniki "Circuit Breaker" w kontekście architektury mikro-serwisów, jak przyczyniają się do odporności?
Bezpieczniki to wzorzec projektowy używany w mikro-serwisach do zarządzania awariami w zdalnych usługach lub 
zależnościach.
...
214) Przepływ rekompensacji w sagach opartych na choreografii?
...
Początkowy przepływ: "1 - 2 - 3 - 4 - 5 do n". Transakcje rekompensacyjne: "n - 5 - 4 - 3 - 2 - 1".

215) Wzorzec choreografii?
...
216) Czym jest przechwytywanie zmiany danych "Change Data Capture"?
...
W Postgres dziennik transakcji nazywa się dziennikiem przed-zapisowym "Write Ahead Log, W A L".
...
217) Wyzwania i rozwiązania związane z utrzymywaniem spójności danych?
...
220) Czy możesz wyjaśnić wyzwania związane z zarządzaniem danymi w architekturze mikro-serwisów, strategię i wzorce?
...
"C Q R S": oddzielenie odpowiedzialności za polecenia (zapis) i zapytania (odczyt) w różne komponenty.
...
221) Czym jest pamięć podręczna "Caching"?
...
222) Logika biznesowa w Czystej architekturze?
...
223) Czym jest sieć mostkowa "Bridge network"?
Tworzy sieć do łączenia z nazwami hostów.

224) Ograniczone zdarzenie w "D D D" "Bounded Event D D D)"?
...
225) Testowanie granic (Boundary Testing) i jego znaczenie w testowaniu oprogramowania?
...
Robustność "Trwałość": Aplikacja powinna radzić sobie z ekstremalnymi danymi wejściowymi w sposób elegancki. 
Bezpieczeństwo.

226) Testowanie granic?
Polega na testowaniu wartości, które znajdują się na granicy lub krawędzi prawidłowych zakresów wejściowych lub warunków. 
Celem jest ujawnienie problemów, takich jak błędy "off-by-one", przepełnienie bufora i nieoczekiwane zachowanie w pobliżu 
limitów. Aby to osiągnąć: dokładność, bezpieczeństwo, jakość.

227) Testowanie "black box" kontra testowanie "white box", kiedy stosować?
...
228) Przykład testowania "black box"?
...
Test poprzez wdrożenie mikro-serwisu i sprawdzenie jego logów oraz metryk.
Zapewnia to, że wdrożenie (deployment), logowanie i monitorowanie są zgodne z określoną architekturą makro.
Taki test nazywa się testem czarnej skrzynki, który sprawdza zachowanie mikro-serwisu z zewnątrz.

230) Opis algorytmu bankiera?
...
232) Dostępność (Availability): jak ją osiągnąć?
...
233) Automatyczne testowanie w rozwoju oprogramowania i jego zalety?
Zalety:
a) Wydajność.
b) Spójność.
...
234) Mikro-usługa asynchroniczna?
Mikro-usługa jest asynchroniczna, jeśli:
a) Nie wysyła żądania do innych mikro-usług podczas przetwarzania żądań.
b) Wysyła żądanie do innych mikro-usług podczas przetwarzania żądań, nie czekając na wynik.

235) Opis architektury rozproszonej?
...
jak architektura klient-serwer, architektura peer-to-peer, architektura mikro-usług oraz architektura oparta na chmurze.
...
236) Wersjonowanie API w architekturze mikrousług?
...
237) Bramy API (Definicja + Wyzwania)?
...
238) Rozwiązania bram API i technologie chmurowe?
a) Netflix Zuul.
b) Amazon API Gateway.
c) Google Cloud Platform API Gateway.
d) Microsoft Azure API Management.

239) Przykład definicji API?
REST API przez HTTP:
...
241) Formaty danych Apache Kafka?
...
242) Zatwierdzanie offsetu - "Commit Offset", w Apache Kafka?
...
245) Zalety i wyzwania korzystania z architektury mikro-usług w rozwoju systemów rozproszonych?
Zalety mikro-usług:
a) Skalowalność.
b) Modularność.
c) Elastyczność.
...
246) Jakie są niektóre z celów brokera wiadomości - "Message Broker"?
...
247) Jaki jest główny cel równoważnika obciążenia w systemie?
Głównym celem równoważnika obciążenia jest rozdzielanie ruchu między grupą serwerów uruchamiających tę samą usługę.

248) Jakie są potencjalne wady równoważenia obciążenia opartego na "D N S"?
Ekspozycja adresów IP serwerów dla klientów.
Brak aktywnego monitorowania stanu serwerów.

249) Jaka jest kluczowa różnica między balanserami obciążenia sprzętowymi a programowymi?
...
250) Jaka jest główna korzyść z Global Server Load Balancers - "G S L B", w kontekście wydajności systemu?
...
251) Jaką rolę odgrywa Globalny Balancer Obciążenia Serwera - "G S L B", w sytuacjach odzyskiwania po awarii?
Przekierowuje użytkowników do różnych lokalizacji w celu zwiększenia dostępności (higher availability).

252) Jakie jest najdokładniejsze stwierdzenie dotyczące bramy API?
Brama API przekierowuje żądania użytkowników do różnych mikro-serwisów na podstawie API żądania.

253) Proszę wybrać funkcje, które typowo oferuje brama API.
a) Ograniczenie liczby żądań. 
b) Tłumaczenie protokołów.
c) Monitorowanie. 
d) Autoryzacja.

254) Jakie są korzyści z uczynienia aplikacji bezstanową?
a) Mniejszy ślad pamięci aplikacji.
b) Możemy łatwo skalować nasz system poziomo, uruchamiając grupę identycznych instancji aplikacji za balanserem obciążenia.
c) Bezproblemowa integracja i wdrożenie z protokołem HTTP.

255) Jaki jest główny cel "C D N"?
Przyspieszenie dostarczania treści statycznych do użytkowników końcowych.

256) W jaki sposób "C D N" rozwiązuje problem opóźnienia - "Latency", gdy użytkownik znajduje się daleko od centrum danych 
naszego systemu?
Treści statyczne są przechowywane na serwerach brzegowych "C D N", które są fizycznie bliżej użytkownika lub są 
lepiej umiejscowione pod względem sieciowym.

257) W jaki sposób "C D N" poprawia dostępność naszego systemu?
...
258) Nasi eksperci ds. bezpieczeństwa zostali powiadomieni, że nasz system jest pod atakiem "D D O S" - "Distributed 
Denial of Service".
...
Bazy danych SQL są zoptymalizowane pod kątem efektywnego przechowywania. Bazy danych "No-SQL" są zoptymalizowane pod 
kątem efektywnych zapytań. Bazy danych "No-SQL" są łatwiejsze do skalowania poziomego (horizontal scaling) niż SQL.

263) Jakie operacje w bazach danych są zazwyczaj bardziej ograniczone w bazach "No - SQL" niż w bazach SQL?
Transakcje ACID i operacje łączenia - "Join Operations".

264) Przykład zastosowań dla bazy danych grafowej:
...
270) Dlaczego operacje idempotentne są preferowane nad operacjami nieidempotentnymi?
...
271) Przykłady scenariuszy, w których musimy używać paginacji API - "API Pagination"?
...
272) Jakie jest poprawne stwierdzenie porównujące "REST" i "R-P-C"?
"R-P-C" wygląda i działa bardziej jak wywołanie lokalnej metody niż wywołanie API "REST". Zdalne wywołanie procedury
...
273) Stwierdzenia dotyczące wywołań "R-P-C" w porównaniu do wywołań lokalnych metod?
Wywołania "R-P-C" są zazwyczaj wolniejsze niż wywołania lokalnych metod. Wywołania R-P-C są zazwyczaj mniej 
niezawodne niż wywołania lokalnych metod. Wywołania R-P-C abstrahują wywołanie - "Abstract Away", sieciowe do zdalnej 
usługi, co wymaga zakodowania danych w formacie binarnym i oczekiwania na pakiety przemieszczające się przez sieć. 
Dlatego wywołania "R P C" są mniej niezawodne i wolniejsze niż wywołanie lokalnej metody.

274) Stwierdzenie dotyczące API "REST":
"REST" to zestaw ograniczeń architektonicznych i najlepszych praktyk do definiowania interfejsów API dla sieci.

275) Jedną z podstawowych zasad API RESTful jest bezstanowość. Jaką cechę jakościową ta zasada zapewnia naszemu systemowi?
...
276) Jedną z podstawowych zasad API RESTful jest możliwość buforowania.
Serwer musi pośrednio lub bezpośrednio zdefiniować odpowiedź jako buforowalną lub niebuforowalną przez klienta. 
W API REST każda odpowiedź musi być oznaczona jako buforowalna (cacheable) lub niebuforowalna, aby klient wiedział, 
...
277) Co oznacza skrót "M-T-B-F"?
Średni czas między awariami - "Mean Time Between Failures".

278) Co oznacza skrót "M-T-T-R"?
Średni czas do wyzdrowienia - "Mean Time To Recovery".

279) Zgodnie z definicją odporności na błędy (fault tolerance), co się dzieje podczas awarii w systemie "odpornym na błędy"?
System będzie kontynuował działanie na tym samym lub obniżonym poziomie wydajności.

280) Jakie są podstawowe zasady odporności na błędy?
a) Zapobieganie awariom - "Failure Prevention". 
b) Wykrywanie awarii - "Failure Detection". 
c) Odzyskiwanie po awarii - "Recovery FFrom Failure".

281) Jaki jest przykład redundancji przestrzennej w kontekście zapobiegania awariom?
Uruchamianie usługi jako grupy identycznych instancji na różnych komputerach.

282) Jaki jest przykład redundancji czasowej w kontekście odporności na błędy?
Ponowne wysyłanie nieudanych żądań wiele razy.

283) Jaką wadę ma komunikacja synchroniczna między usługami?
Może prowadzić do wysokiej latencji w przypadku operacji długotrwałych.

284) Jaki problem komunikacji synchronicznej rozwiązany jest przez asynchroniczną architekturę opartą na zdarzeniach?
...
285) Jaka jest różnica między architekturą wielowarstwową - "Multi-Layer", a architekturą - "Multi-Tier"?
...
286) Dlaczego umieszczanie logiki biznesowej w warstwie prezentacji jest antywzorem?
...
287) Co nie jest cechą - "Property", zdarzenia w architekturze sterowanej zdarzeniami?
...
288) Opisz korzyści płynące z wzorca "C Q R S" w mikro-serwisach.
W systemach ze złożoną logiką biznesową, gdzie operacje aktualizacji danych znacznie różnią się od operacji odczytu danych, 
...
289) Wzorce projektowe integracji - "Integration Design Patterns"?
...
Kanał martwych wiadomości - "Dead Letter Channel": Obsługuje wiadomości, które nie mogą być przetworzone przez 
...
Konsument pobierający - "Pooling Consumer": Sprawdza wiadomości w regularnych odstępach czasu.
...
Usługi RESTful: Udostępnianie usług za pośrednictwem HTTP przy użyciu zasad REST.
Usługi SOAP: Używanie protokołu SOAP do wymiany ustrukturyzowanych informacji.
...
Wyłącznik obwodu - "Circuit Breaker": Zapobieganie powtarzanym próbom dostępu do usługi, która jest znana z awarii.
...
293) Dlaczego potrzebujemy modelu pub/sub w systemach rozproszonych?
Pub-sub to wzorzec komunikacji używany w systemach rozproszonych, który oferuje wiele korzyści i rozwiązuje kluczowe 
wyzwania. Oto dlaczego pub-sub jest potrzebny w systemach rozproszonych:
a) Rozdzielenie komponentów:
Luźne powiązanie ("Loose Coupling"): Pub-sub rozdziela komponenty w systemie, umożliwiając publikującym i subskrybującym 
niezależne działanie. Publikujący nie muszą wiedzieć, kim są subskrybenci, a subskrybenci nie muszą wiedzieć, 
skąd pochodzą dane. Skalowalność: To rozdzielenie pozwala systemowi na łatwiejszą skalowalność, ponieważ komponenty mogą 
być dodawane lub usuwane bez znaczącej rekonfiguracji.
...
bez czekania na przetworzenie ich przez subskrybentów. To prowadzi do bardziej efektywnego wykorzystania zasobów i 
lepszej wydajności. Architektura oparta na zdarzeniach: Wspiera architekturę opartą na zdarzeniach, w której systemy 
reagują na zdarzenia w czasie rzeczywistym, co jest kluczowe dla responsywnych i interaktywnych aplikacji.
c) Możliwość multi-castu:
Transmisja wiadomości: Pub-sub pozwala na jednoczesne przesyłanie wiadomości do wielu subskrybentów, co czyni go wydajnym 
w scenariuszach, w których ta sama wiadomość musi dotrzeć do wielu komponentów. Efektywna dystrybucja danych: To zmniejsza 
potrzebę wielu komunikacji punkt-punkt, oszczędzając przepustowość i redukując opóźnienia.
...
monitorowania. 
Komunikacja w mikro-serwisach: W architekturze mikro-serwisów pub-sub ułatwia komunikację między usługami bez ścisłego 
powiązania, promując lepszą izolację usług i odporność.

294) Normalizacja i denormalizacja baz danych?
...
295) Wybór odpowiedniej bazy danych?
...
i elastyczności (przykład: "Mongo DB", Cassandra, Redis).

296) Projektowanie schematu bazy danych?
...
297) Indeksowanie w bazach danych?
...
298) Integralność danych w bazach danych?
...
299) Skalowalność baz danych?
Skalowanie pionowe: Zwiększ pojemność pojedynczego serwera (przykład: dodawanie większej ilości CPU, RAM).
Skalowanie poziome: Rozłóż obciążenie na wiele serwerów (przykład: sharding w bazach danych "No-SQL", 
klastrowanie baz danych).

300) Kopiowanie zapasowe i odzyskiwanie w bazach danych?
...
302) Optymalizacja wydajności baz danych?
Optymalizacja zapytań: Pisanie efektywnych zapytań i korzystanie z narzędzi do optymalizacji zapytań oferowanych przez 
bazę danych.
...
303) Monitorowanie i konserwacja baz danych?
Narzędzia monitorujące: Używaj narzędzi monitorujących, aby śledzić wydajność i stan bazy danych(przykład: logi wolnych 
zapytań, pulpity wydajności). Regularna konserwacja (Regular Maintenance): Wykonuj regularne zadania konserwacyjne, 
takie jak aktualizacja statystyk, ponowne indeksowanie i usuwanie starych danych.

304) Zgodność baz danych?
...
305) Czy możesz wyjaśnić koncepcję OAuth?
...
Odpowiedź z tokenem dostępu (Access Token Response): Serwer autoryzacji zwraca token dostępu (opcjonalnie token odświeżania) 
do klienta. Dostęp do zasobu (Access Resource): Klient używa tokena dostępu do żądania chronionego zasobu z serwera zasobów.
...
Niektóre powszechne polityki zastępowania to: Least Recently Used (LRU), Most Recently Used ("M R U"), 
...
307) Rozproszona tablica haszująca (DHT)?
...
311) "Map-Reduce"?
"Map-Reduce" to model programowania oraz powiązana implementacja do przetwarzania i generowania dużych zbiorów danych. 
Został opracowany przez Google i jest kluczowym elementem projektu Apache Hadoop.
Podstawową koncepcją "Map-Reduce" jest podzielenie dużego zbioru danych na mniejsze fragmenty, które są następnie 
...
par klucz-wartość.
"Map-Reduce" jest szeroko stosowane w przemyśle do przetwarzania dużych danych, w tym do takich zadań jak eksploracja danych, 
...
Rozproszona natura "Map-Reduce" sprawia, że jest to popularny wybór do przetwarzania dużych zbiorów danych, które nie mogą 
być obsługiwane przez pojedynczą maszynę.

312) Problem z pętlą for?
...
Oto ogólny przegląd działania load balansera:
a) Klient wysyła żądanie do adresu IP lub nazwy hosta load balansera.
...
e) Serwer przetwarza żądanie i wysyła odpowiedź z powrotem do load balansera.
f) Load balancer przekazuje odpowiedź z powrotem do klienta.
Istnieje kilka typów load balanserów i algorytmów, które można wykorzystać, takich jak:
...
e) Load Balancing na poziomie 7 ("Layer Seven Load Balancing"): rozdziela żądania na podstawie danych warstwy aplikacji, 
...
globalnego load balancingu. Load balansery mogą również oferować inne funkcje, takie jak odciążenie SSL, caching, 
kompresję i kontrole stanu, aby zapewnić, że tylko zdrowe serwery otrzymują ruch.
Istnieje kilka typów load balancsrów, w tym:
a) Load balansery sprzętowe: To fizyczne urządzenia instalowane lokalnie. Zwykle są droższe od load balanserów programowych, 
ale oferują lepszą wydajność i bardziej zaawansowane funkcje. Przykłady to "F5 BIG-IP" i "Cisco ACE".
b) Load balansery programowe: To load balansery działające na oprogramowaniu, które można zainstalować na serwerze lub 
w chmurze. Zwykle są tańsze od load balanserów sprzętowych, ale mogą nie oferować tej samej wydajności lub funkcji. 
Przykłady to "HAProxy", "NGINX" i "Amazon Elastic Load Balancer".
c) Load balansery w chmurze: Te load balansery są oferowane przez dostawców chmury i są zazwyczaj łatwe do skonfigurowania 
i zarządzania. Przykłady to "Amazon Elastic Load Balancer", "Google Cloud Load Balancer" i "Microsoft Azure Load Balancer".

315) Serwery nazw domen?
...
pochodzą z autoryzowanych źródeł, oraz oferując dodatkowe funkcje bezpieczeństwa, takie jak "DNS SEC", które zapewniają, 
...
"nslookup DOMAIN_NAME" zwróci adres IP skojarzony z nazwą domeny.
...
Na przykład, "dig DOMAIN_NAME" zwróci adres IP skojarzony z nazwą domeny.
c) "host": To polecenie jest podobne do "nslookup" i "dig", pozwala na zapytanie serwerów DNS i uzyskanie informacji o 
nazwie domeny. Na przykład, "host DOMAIN_NAME" zwróci adres IP skojarzony z nazwą domeny.
d) "ping": To polecenie pozwala na przetestowanie dostępności hosta, wysyłając pakiety żądań "ICMP" i mierząc czas odpowiedzi. 
Na przykład, "ping DOMAIN_NAME" przetestuje dostępność hosta skojarzonego z nazwą domeny.
e) "traceroute": To polecenie pokazuje ścieżkę, jaką pakiet pokonuje, aby dotrzeć do celu. Jest użyteczne w rozwiązywaniu 
problemów z trasowaniem. Na przykład, "traceroute DOMAIN_NAME" pokaże ścieżkę, jaką pakiet pokonuje, aby dotrzeć do 
...
W systemie Linux konfiguracja zapory jest zazwyczaj przechowywana w pliku konfiguracyjnym "ip-tables", który zazwyczaj 
znajduje się w "/etc/sysconfig/iptables" lub "/etc/iptables/iptables.rules".
Ten plik zawiera reguły, które są ładowane do zapory "ip-tables" podczas uruchamiania systemu.
Innym sposobem na zachowanie reguł zapory jest użycie usługi, która ładuje reguły "ip-tables" przy uruchamianiu, 
takiej jak "ip-tables-persistent" (dla Ubuntu i Debiana) lub "firewalld" (dla Red Hat i CentOS), która zapewnia wygodny 
...
dlatego zawsze warto zapoznać się z dokumentacją dla konkretnej dystrybucji, aby określić lokalizację pliku konfiguracyjnego 
"ip-tables".
...
318) Multi-core, nowoczesne ograniczenia współbieżności ("Modern Concurrency Constraints")?
...
319) Przestrzeń wymiany ("Swap Space")?
...
Partycja wymiany ("Swap Partition"): Dedykowana partycja na urządzeniu pamięci masowej, specjalnie zarezerwowana dla 
"Swap Space". Plik wymiany ("Swap File"): Zwykły plik w systemie plików, który pełni tę samą funkcję co partycja wymiany.
...
320) Pamięć podręczna ("Memory Cache")?
...
do danych, ponieważ CPU nie musi mieć dostępu do pamięci głównej ani urządzenia magazynującego.
...
321) Harmonogramowanie ("Scheduling")?
...
322) Jakie zasoby są potrzebne procesowi i wątkowi?
...
324) Zakleszczenie (Deadlock) i jak go unikać?
...
327) Drzewa Czerwono-Czarne ("Red-Black Tree")?
...
stripe'owaniem blokad ("Lock Striping"), aby osiągnąć wysoką współbieżność. Używa struktury danych w postaci drzewa 
balansującego, znanej jako drzewo czerwono-czarne, do przechowywania elementów, gdy rozmiar mapy rośnie.
...
328) Różne poziomy testowania (testy jednostkowe, integracyjne, systemowe i akceptacyjne)?
a) Testy Jednostkowe.
...
Narzędzia: "J-Unit", "N-Unit", "py-test" (do testów integracyjnych) lub specjalistyczne narzędzia do testowania 
integracji, takie jak Postman do testowania API.
...
329) Różne fazy cyklu życia oprogramowania (SDLC)?
a) Planowanie.
...
330) Kluczowe zasady ciągłej integracji/ciągłego wdrażania (CI/CD)?
...
331) Czym jest cykl życia rozwoju oprogramowania (SDLC)?
...
332) Wyjaśnienie różnych typów złączeń SQL ("SQL Joins")?
...
333) Czym są klucz główny i klucz obcy w SQL?
...
334) Jaka jest różnica między bazami danych SQL a "No SQL"?
Bazy danych SQL:
...
Bazy danych "No SQL":
Przykłady: "Mongo-DB", "Cassandra", "Couch DB", "Redis".
...
modelu "A P".
Podsumowując, bazy danych SQL są strukturalne i niezawodne, podczas gdy bazy danych "No-SQL" oferują elastyczność i 
skalowalność dla różnorodnych typów danych.

335) Jakie są różnice między "git fetch" a "git pull"? 
...
336) Jak działa biblioteka RestAssured? Podaj przykład. 
...
337) Czym jest ciało żądania / ładunek ("payload") w API? 
...
338) Jaka jest różnica między parametrem ścieżki ("path parameter") a parametrem zapytania ("query parameter")? 
Parametr ścieżki jest częścią samej ścieżki URL. Parametr zapytania to dane w postaci pary klucz-wartość, które można 
dołączyć do naszego żądania, aby przekazać dane do serwera. Są one dołączane na końcu URL po znaku "?" i są oddzielane 
znakiem "&".

339) Czym są parametry zapytania w API? 
...
340) Jak działa autoryzacja w API?
...
341) Jakie kody statusu odpowiedzi znasz? 
...
342) Jaka jest różnica między PUT a PATCH?
...
344) Jaka jest różnica między "SOAP" a "REST" API? 
...
"formatowane" podczas wymiany informacji. Kluczowe różnice między HTTP a HTTPS dotyczą bezpieczeństwa. HTTPS dodaje 
...
348) Czym jest wzorzec projektowy singleton i dlaczego warto go używać w swoim frameworku? 
...
349) Jakie rodzaje raportów generujesz z swojego frameworku? 
Mój framework generuje raporty "maven-cucumber-reporting" (raporty Cucumber).

350) Jak zarządzasz danymi testowymi w swoim frameworku? 
...
351) Jak twój kod działa jako część "CI" i jaka jest twoja strategia "CI"? 
...
352) Czym jest Ciągła Integracja, "Continuous Integration (CI)"? 
"CI" to praktyka w zakresie rozwoju oprogramowania w "Dev Ops", w której deweloperzy regularnie łączą swoje zmiany w kodzie
...
353) Jak możesz przekazać wartość z polecenia "Maven" do swojego kodu? 
Polecenie "Maven" z właściwością systemową: "m-v-n test -Denv=PROD".

354) Jakie cykle życia "Maven" znasz? 
...
355) Czym jest "Maven"? Jaką rolę odgrywa w twoim frameworku? 
...
pakowanie binarów i tworzenie artefaktów.
...
356) Jaka jest różnica między asercją twardą ("hard assert") a asercją miękką ("soft assert")? 
...
357) Dlaczego potrzebujesz "JUnit" / "TestNG" w swoim frameworku? 
...
359) Jak uruchomić tylko scenariusze testów smoke lub tylko scenariusze testów regresyjnych w Cucumber?
Możemy użyć tagów Cucumber. "mvn test -Dcucumber.options=--tag @smoke".

360) Jak działa parametryzowany krok w Cucumber? 
...
362) Czym jest "Background" w Cucumber? 
...
363) Czym jest "Scenario Outline" w Cucumber? 
...
364) Czym jest definicja kroku ("Step Definition")? 
...
365) Czym są słowa kluczowe "Feature", "Scenario" i "Steps"? 
...
366) Czym jest klasa runner? Czy możesz opisać, co się w niej znajduje? 
Klasa runner to punkt wykonania dla Cucumber. Łączy pliki Feature z ich definicjami kroków.

367) Jakie są główne komponenty Cucumber? 
...
371) Czym są "SauceLabs" i "BrowserStack"? 
...
373) Czym jest "Page Object Model (POM)"? 
...
374) Co robi klasa "Actions" w "Selenium"? 
...
375) Jak wpisać tekst w polu wejściowym w "Selenium"? 
Robimy to za pomocą metody "sendKeys(text)".

376) Jak pracować z oknami popup w Selenium? 
...
377) Jak pracować z rozwijanym menu "UI" w Selenium? 
Gdy rozwijane menu jest oparte na tagu select, możemy użyć klasy Select, aby z nim pracować.

378) Jak pracować z "iFrame" w Selenium? 
Najpierw przełącz się na "iFrame", używając metody "switch-To()". Możesz przełączać się według indeksu, id lub nazwy. 
Gdy zakończysz swoje zadanie w "iFrame", możesz wrócić, używając "defaultContent".

379) Jak pracować z wieloma oknami w Selenium? 
...
380) Co sprawdzasz, jeśli element istnieje w DOM? 
...
381) Jaka jest różnica między "findElement" a "findElements"? 
...
382) Jaka jest różnica między czekaniem "implicit" a "explicit"? 
...
383) Czym jest czekanie "fluent" w Selenium? 
...
386) Jaka jest różnica między "xpath" a "CSS"? 
...
387) Jaka jest różnica między ścieżkami "absolute" a "relative"? 
...
388) Który lokalizator Selenium preferujesz? 
...
389) Jakie rodzaje lokalizatorów znasz w Selenium?
ID, Name, Class name, xpath, CSS, link text and partial link text, tag name.

390) Czym jest Model Obiektowy Dokumentu (DOM) w UI? 
...
391) Jakie są zalety i wady Selenium? 
Zalety: 
...
392) Jak przeprowadzasz testy regresyjne w swoim projekcie? 
...
393) Co robisz, gdy twój zautomatyzowany zestaw testów zawiedzie? 
...
394) Jak określasz, które testy zautomatyzować? 
...
umieszczone w tej samej minucie (problem z UI). Po zmianie wersji jednej z bibliotek, odpowiedź generowana zmieniła swoją
kolejność, było to spowodowane tym, że nowa biblioteka wymuszała użycie HashMap, gdzie wcześniej używaliśmy LinkedHashMap.

397) Czym jest testowanie wydajności? 
...
398) Jakie różne techniki testowania znasz i stosujesz? 
...
399) Jak określasz, które przypadki testowe powinny być częścią zestawu testów regresyjnych? 
Przypadek testowy powinien dotyczyć ważnej funkcjonalności. Powinien być to przypadek testowy powtarzalny.

400) Czym jest zestaw testów regresyjnych i dlaczego jest potrzebny? 
...
401) Czym jest Macierz Śledzenia Wymagań ("Requirements Traceability Matrix (RTM)") w testowaniu? 
...
402) Czym jest cykl życia defektu? 
...
403) Czym jest cykl życia testowania oprogramowania (STLC) w Twoim zespole? 
...
404) Czym jest przypadek testowy i co zawiera doskonały przypadek testowy?
...
405) Czym jest plan testów i co zawiera dobry plan testów?
...
410) Rozwój napędzany testami (TDD)?
...
411) Frameworki mokujące?
Ekosystem Javy obejmuje szereg frameworków mokujących.
Cechy ("Features"): Minimalizują ręczne kodowanie moków, wykorzystują adnotacje. Ustawiają oczekiwania dotyczące odpowiedzi moków. 
Weryfikują wywołania metod, w tym liczbę wywołań. Obsługa programowa do zgłaszania wyjątków.
"Spring-boot-starter-test": Zależność przejrzysta dla mokowania.

412) Mocki z Mockito i Spring Boot?
Technika używania podwójnych testów jest znana jako "mokowanie".
Korzyści z mokowania: Umożliwia testowanie danej klasy w izolacji. Testuje interakcje między daną klasą a jej zależnościami. 
Minimalizuje konfigurację/dostępność zależności. Na przykład DAO, baza danych, REST API itd.: Możemy zmokować DAO, aby 
zwróciło odpowiedź. Możemy zmokować REST API, aby zwróciło odpowiedź.

413) Wsparcie Spring Boot dla testów jednostkowych?
Czego potrzebujesz do testowania jednostkowego w Spring Boot? Dostępu do kontekstu aplikacji Spring. Wsparcia dla 
wstrzykiwania zależności Spring. Uzyskiwania danych z pliku "application.properties" Spring. 
Wsparcia dla obiektów mokujących dla aplikacji webowych, danych, REST API. 
...
414) Testowanie z użyciem refleksji?
...
Używaj "@MockBean" w Spring Boot, gdy potrzebujesz wstrzykiwać moki oraz regularne beany z kontekstu aplikacji.

416) Testy parametryzowane?
"J Unit" oferuje "@ParametrizedTest". Uruchamia test wielokrotnie, dostarczając różne wartości parametrów.
Źródła wartości dla "@ParametrizedTest": "@ValueSource", "@Csv-Source", "@Csv-FileSource", "@EnumSource", "@MethodSource".

417) Co oznacza skalowanie pionowe?
...
418) Co wymaga skalowanie poziome?
...
419) Host bazy danych, który jest replikowany do kopii zapasowej, która jest zawsze gotowa do przejęcia, jest przykładem?
...
422) Rozproszone bazy danych "No-SQL" z węzłem głównym, który rozdziela transakcje, znajdują się po której stronie trójkąta CAP? 
...
423) Jaką politykę usuwania danych należy zastosować przy projektowaniu pamięci podręcznej, która odrzuca dane, 
...
424) Jeśli musisz zminimalizować opóźnienie klientów przy pobieraniu danych statycznych z całego świata bez względu na koszty?
"C D N": Sieci dostarczania treści ("C D N") są zaprojektowane w celu rozwiązania problemu globalnego ruchu i minimalizacji 
opóźnień spowodowanych długimi skokami sieciowymi. Load balansery i technologie "cache", takie jak Redis, mogą 
również być częścią projektów o niskim opóźnieniu, ale nie są szczególnie ukierunkowane na problem globalnego ruchu.

425) SLA wynoszące 100 ms przy "three nines" oznacza? 
Mój system odpowie w mniej niż 100 milli sekundach w 99,9% przypadków.

426) W HDFS serwer odpowiedzialny za koordynację żądań nazywa się? 
...
427) Wzorce projektowe! 
Zestaw najlepszych praktyk, rozwiązań, ustrukturyzowane podejście do problemów w projektowaniu oprogramowania. 
"O O P", funkcyjne, chmurowe, integracyjne, odporność, architektura oparta na zdarzeniach.

428) Wzorce integracji? 
...
431) Orchestrator Pattern?
Agregator + dodatkowa logika biznesowa do zapewnienia przepływu pracy.

432) Orchestrator Pattern ("Chained / Sequential workflow")?
...
436) Circuit Breaker Terms?
"CLOSED": Usługa zależna jest działająca. Wszystkie żądania są wysyłane.
"OPEN": Usługa zależna jest niedostępna. Żądania nie są wysyłane.
