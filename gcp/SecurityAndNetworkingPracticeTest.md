...
**Question 18: VPCs are GLOBAL resources.**
...
```
* The target can be all instances in a network, instances with particular network tags, or instances using a 
...
```
Scopes are permissions granted to VM instances. 
Scopes in combination with IAM roles assigned to service accounts assigned to the VM instance determine what operations 
the VM instance can perform. 
...
```
The three global load balancers are HTTP(S), SSL Proxy, and TCP Proxy. 
Internal TCP/UDP is a regional load balancer.
```
...
**Question 38: PREMIUM TIER routes traffic only over Google’s global network.**
...
* Predefined roles do not implement defense in depth by themselves but could be used with other security 
controls to implement defense in depth.
```

**Question 44: STANDARD TIER uses the public Internet network to transfer data between Google data centers**
...
**Question 47: You want to implement interproject communication between VPCs. Which feature of VPCs would you use to 
implement this?**
...
* Identity access management is used for authenticating and authorizing users; it is not relevant to network 
controls between subnetworks. 
